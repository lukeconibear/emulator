...
sim_list = list(map(str, list(range(1, 51))))
sim_list = ['t' + sim for sim in sim_list]
training_data = pd.read_csv('latin_hypercube_inputs_train.csv', header=0).values
    ...
    for emis_file in emis_files:
        for index, scenario in enumerate(scenarios):
            ...
            if (scenario in emis) == True:
				...
				sec = sec.where(cond=sec.china!=1, other=sec * training_data[int(sim.replace('t', '')) - 1][index]) # if condition (not in china) preserve, otherwise (in china, and scale)
                emis[scenario] = sec
